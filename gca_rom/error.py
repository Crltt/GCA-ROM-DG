...
def compute_analytic_error(res,dataset, scaler, HyperParams):
    """
    ...
    a_sol_list = list()
    a_sol_matr = np.empty((189,1220))
    for snap in range(res.shape[0]):
      a_sol = np.sin(np.pi*x)*np.sin(np.pi*y)*np.cos(np.sqrt(2)*snap)
      a_sol_list.append(a_sol)
      a_sol_matr[snap,:] = a_sol
   

    a_sol_tensor = torch.tensor(a_sol_matr).reshape(189,1220,1)
